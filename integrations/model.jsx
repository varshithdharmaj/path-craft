 ...
 * 
 * This module provides AI-powered course generation using Google's Gemini API.
 * The client is initialized lazily to prevent errors when the API key is not set.
 */
import {
  GoogleGenerativeAI
...
// Lazy initialization - chat sessions are created when needed
function getCourseLayoutChat() {
...
export const GenerateCourseLayout_AI = {
  async sendMessage(prompt) {
    const chat = getCourseLayoutChat();
    return await chat.sendMessage(prompt);
  }
};

function getChapterContentChat() {
  const model = getGeminiClient();
  return model.startChat({
    generationConfig,
    history: [
...
export const GenerateChapterContent_AI = {
  async sendMessage(prompt) {
    const chat = getChapterContentChat();
    return await chat.sendMessage(prompt);
  }
};